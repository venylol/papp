...
def _auto_detect_db_dir_windows():
    ...
    部分微信 4.x 版本的 ini 只有 MyDocument: 标记，不会直接保存
    数据目录。因此同时检查 Windows 文档目录、已知用户目录和磁盘根目录
    下的常见 xwechat_files 位置。只检查有限的固定层级，不递归扫描整块磁盘。
    """
    appdata = os.environ.get("APPDATA", "")
    config_dir = os.path.join(appdata, "Tencent", "xwechat", "config")

    def add_unique(items, seen, path):
        if not path:
            return
    ...
    def add_ini_paths(items, seen):
        """从 ini 中提取目录；兼容纯路径、键值和 MyDocument: 标记。"""
        ...
        for ini_file in glob.glob(os.path.join(config_dir, "*.ini")):
            try:
                ...
                if not candidates and os.path.splitdrive(content)[0]:
                    candidates = [content]
                for candidate in candidates:
                    add_unique(items, seen, candidate.rstrip(" ;,"))
            except OSError:
                continue

    def add_known_roots(items, seen):
        """加入不依赖微信版本的 Windows 常见数据根目录。"""
        ...
        for candidate in (
            ...
            add_unique(documents, document_seen, candidate)

        # 用户可能把 Windows 文档目录重定向到 OneDrive 或其他位置。
        try:
            import winreg
            ...
            with winreg.OpenKey(
                ...
                add_unique(documents, document_seen, personal)
        ...
        for docs in documents:
            for name in (
                "xwechat_files",
                "WeChat Files",
                os.path.join("Tencent", "xwechat_files"),
            ):
                add_unique(items, seen, os.path.join(docs, name))

        for base in (home, appdata, os.environ.get("LOCALAPPDATA", "")):
            for name in ("xwechat_files", "WeChat Files"):
                add_unique(items, seen, os.path.join(base, name))

        # 自定义微信存储位置常见于 D:\xwechat_files 或 D:\某目录\xwechat_files。
        # 只检查每个磁盘根目录及下一层，避免递归扫描用户的整块磁盘。
        for letter in string.ascii_uppercase:
            ...
            for pattern in (
                ...
                for match in glob.glob(pattern):
                    add_unique(items, seen, match)

    # 从 ini 文件中找到有效的目录路径，再补充常见默认/自定义位置。
    data_roots = []
    data_root_seen = set()
    add_ini_paths(data_roots, data_root_seen)
    add_known_roots(data_roots, data_root_seen)
    ...
    # 近期开启/写入的账号通常会更新 cache、temp、session 或数据库文件。
    # 用有限的顶层目录时间排序，避免多个历史账号时要求用户先查路径。
    def activity_mtime(path):
        ...
        targets = [
            ...
            os.path.join(account_dir, "session"),
            os.path.join(account_dir, "resource"),
            os.path.join(account_dir, "message"),
        ]
        mtimes = []
        for target in targets:
            try:
                mtimes.append(os.path.getmtime(target))
            except OSError:
                pass
    ...
    candidates.sort(key=activity_mtime, reverse=True)
    if len(candidates) > 1 and os.environ.get("WECHAT_DECRYPT_CHOOSE_ACCOUNT") != "1":
        print(f"[*] 检测到 {len(candidates)} 个微信账号目录，已自动选择最近活动的账号")
        return candidates[0]
